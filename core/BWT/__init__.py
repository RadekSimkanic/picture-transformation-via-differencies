...
import pstats
import sys
# https://github.com/RadekSimkanic/Compression-and-transformation/tree/master/BWT
...
class MyZip:
    ...
    def next(self):
        if self.i >= self.length:
            self.i = 0
            raise StopIteration
        t = (self.l1[self.i], self.l2[self.i])
        self.i += 1
        return t
...
class RotationList:
    ...
    def __lt__(self, other):
        ...
        return False
        #return tuple(self) < tuple(other) #list(self) < list(other)
    def __len__(self):
        return self.length

    def __cmp__(self, other): #is the same self and other
        if len(set(list(self)) ^ set(list(other))):
            return False
        return True
...
class BWTOwnAlphabet:
    def __init__(self, inline_list_of_data, own_alphabet, is_transformed_number_data = False):
        if not isinstance(inline_list_of_data, (list, tuple)):
            print str(type(inline_list_of_data))
            raise TypeError, "The type must be a list or tuple"
        if not isinstance(own_alphabet, (list, tuple)):
            raise TypeError, "The type must be a list or tuple"
        ...
        if is_transformed_number_data:
            ...
            self.inline_number_list_of_data = self.__transformDataToNumbers()

    def __transformDataToNumbers(self):
    ...
    def __transformDataFromNumbers(self, inline_number_list_of_data):
        inline_data = map(
    ...
    def encode(self, numbers_output = False):
        ...
        if numbers_output is False:
            encode_data = self.__transformDataFromNumbers(encode_data)
    ...
    def decode(self, synchronization_index, numbers_output = False):
        ...
        counter = self.__counter(encoded_data)
        ...
        if numbers_output is False:
            decoded_data = self.__transformDataFromNumbers(decoded_data)

        return decoded_data

    def __counter(self, lst):
        tmp_counter = {}
        count = []
        for char in lst:
            c = tmp_counter.get(char, 0) + 1
